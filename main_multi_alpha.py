    ...
    for alpha in alpha_list:
        ...
        for trial in range(NUM_TRIALS):
            ...
            for node in nodes:
                ...
                node.set_generation_protocol(CONTINUOUS_SCHEME, alpha)
    
            # Generate request node pair queue
            # pair_queue = gen_pair_queue(traffic_mtx, NET_SIZE, QUEUE_LEN, rng, rng)
            pair_queue = [(18, 2) for i in range(QUEUE_LEN)]  # a queue of identical requests
        ...
        num_requests = min([len(latencies_list[i]) for i in range(NUM_TRIALS)])
        ...
        # construct error
        high_percentile = np.zeros(num_requests)
